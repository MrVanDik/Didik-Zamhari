...
@section('content')
    ...
    <div class="flex-grow-1">
        ...
        <nav class="navbar navbar-light">
            <span class="navbar-brand mb-0 h5 fw-bold">Edit Data Pasien</span>
            <div class="d-flex align-items-center">
                <span class="text-muted me-2"><i class="fas fa-user-circle me-1"></i> {{ Auth::user()->name ?? 'Petugas' }}</span>
            </div>
        </nav>
        ...
        <div class="content">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0"><i class="fas fa-user-edit me-2"></i>Form Edit Pasien</h5>
                </div>
                <div class="card-body">
                    @if($errors->any())
                    ...
                    <form action="{{ route('pendaftaran.pasien.update', $pasien->id_pasien) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="id_pasien" class="form-control @error('id_pasien') is-invalid @enderror" required 
                                       placeholder="Masukkan NIK pasien" maxlength="16" readonly
                                       oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                                       value="{{ old('id_pasien', $pasien->id_pasien) }}">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="tgl_lahir" class="form-control @error('tgl_lahir') is-invalid @enderror" 
                                       max="{{ date('Y-m-d') }}"
                                       value="{{ old('tgl_lahir', $pasien->tgl_lahir ? $pasien->tgl_lahir->format('Y-m-d') : '') }}">
                                @error('tgl_lahir')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="id_wilayah" class="form-select @error('id_wilayah') is-invalid @enderror">
                                    ...
                                    @foreach($wilayah as $w)
                                        <option value="{{ $w->id_wilayah }}" {{ old('id_wilayah', $pasien->id_wilayah) == $w->id_wilayah ? 'selected' : '' }}>
                                            {{ $w->nama_wilayah }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" value="{{ $pasien->usia }} tahun" readonly>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="alamat" class="form-control @error('alamat') is-invalid @enderror" rows="3" 
                                      placeholder="Masukkan alamat lengkap pasien">{{ old('alamat', $pasien->alamat) }}</textarea>
                            @error('alamat')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
